...
public class Statistics implements IStatistics {
        ...
        if(args[0].equals("-d")){
            ...
            if(args.length != (index + 2) && args[index].equals("-m")){
                while(args.length != (index + 2) && args[index+1].charAt(0) != '-'){
                    index++;
                    switch (args[index]){
                        case "median": median = true; break;
                        case "mean": mean = true; break;
                        case "sd": sd = true; break;
                        case "quantile=0.25": quantile25 = true; break;
                        case "quantile=0.25,0.75": quantile75 = true; break;
                        case "quantile=0.25-0.75": quantile2575 = true; break;
                        case "range": range = true; break;
                        case "iqr": iqr = true; break;
                        default: break;
                    }
                }
                index++;
            ...
            if(args.length != (index + 2) && args[index].equals("-p")){
                while(args.length != (index + 2) && args[index+1].charAt(0) != '-'){
                    ...
                    switch (args[index]){
                        ...
                        case "hist": hist = true; break;
                        case "strip": strip = true; break;
                        default: break;
                    }
                }
                index++;
    ...
    public void buildMeasureRFile() {
        ...
        if (quantile2575 || quantile25 || quantile75) {
            ...
            if (quantile75) {
                for (int i = 0; i < scenarios.length; i++) writer.format("quantile(s" + scenarios[i] + ",probs = c(0.25,0.75))\n");
                writer.format("\n");
            }

            if (quantile2575) for (int i = 0; i < scenarios.length; i++)
                writer.format("quantile(s" + scenarios[i] + ",probs = c(0.25,0.50,0.75))\n");
        }

        if (range) {
            //writes range
            writer.format("\n# range\n");
            for (int i = 0; i < scenarios.length; i++) writer.format("c(max(s" + scenarios[i] + ") − min(s" + scenarios[i] + "))\n");
        }
    ...
    public void buildTTestRFile() {
        if (test) {
            ...
            String strMean = "";
            if (scenarios.length == 1) strMean = "mean(s01)";
            else {
                strMean = "c(";
                for (int i = 0; i < (scenarios.length - 1); i++) {
    ...
    public void buildMostFrequentFitnessValuesRFile() {
        if (mff) {
            ...
            for (int i = 0; i < (scenarios.length - 1); i++) writer.format("sort(table(s" + scenarios[i] + "),decreasing=TRUE)[1]\n");
